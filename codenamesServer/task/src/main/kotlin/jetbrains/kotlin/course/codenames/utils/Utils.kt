...
object Utils {
    ...
    val uniqueKeyCardGenerator = KeyCardGenerator {
        var newCellList = KeyCardType.values().map { key -> List(key.number) { KeyCardCell(key) } }.flatten()
        do {
            newCellList = newCellList.shuffled()
        } while (newCellList in previousAttempts)
        previousAttempts.add(newCellList)
        return@KeyCardGenerator newCellList
    }
}